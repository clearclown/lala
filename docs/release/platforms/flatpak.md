...
1. **Flathub申請のためのGitHub操作**（初回のみ、10-15分）
   - https://github.com/flathub/flathub にアクセス
   - Issue を作成して新アプリリポジトリをリクエスト
   - Flathubチームからリポジトリが作成される（数時間～1日）

2. **プルリクエスト作成とレビュー対応**（各バージョン、数日～数週間）
   - GitHub上で https://github.com/flathub/flathub にPR作成
   - レビュアーからのコメントに対応
   - 承認されるまでやり取り

**ローカルでのビルド・テストは全てCLIで完結します！**
**Flathub申請は高度な機能でオプションです（個人配布も可能）。**
