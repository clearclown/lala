...
1. **コード署名証明書の購入**（オプション、年間$100-400）
   - DigiCert、Sectigo、GlobalSignなどのプロバイダーサイトにアクセス
   - Code Signing Certificate を購入
   - 身元確認プロセス（企業の場合は書類提出）
   - 証明書のダウンロード（.pfx形式）

   ⚠️ **署名なしでも配布可能ですが、Windows Defenderで警告が表示されます**
   ⚠️ **個人開発者にはコストが高いため、最初はスキップ推奨**

**インストーラーのビルド・配布は全てCLI（cargo wix / makensis）で完結します！**
